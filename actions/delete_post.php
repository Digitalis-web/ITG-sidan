<?php
	...
	if($creator_id == $_SESSION['account_id'] || $_SESSION['log_in'] > 1) {
		mysqli_query($con, "DELETE FROM post WHERE post_id = '$post_id'") or die (mysqli_error($con));
		
		// removes all the images from this post
		mysqli_query($con, "DELETE FROM post_image WHERE post_id = '$post_id'") or die (mysqli_error($con));
	}
